...
// Param from launch file
    ...
    double _Vel, _Acc;// 4.0 3.0
    int    _dev_order, _min_order;// 4 3
...
{ 
    ...
    */
    double _Vel = 1.0, _Acc = 1.0;
    double veldiv = 1.0/_Vel;
    double accdiv = 1.0/_Acc;
    //v0 = 0
    double Time1 = _Vel*accdiv*2;
    double Distance = _Vel*_Vel*accdiv;
    ...
    for(int i = 0;i<Path.rows()-1;++i){
        ...
        if(dist<=Distance){
            t = 2*std::sqrt(dist*accdiv);
        }else{
            t = Time1+(dist-Distance)*veldiv;
        }
        time(i) = t;
    }
    return time;
